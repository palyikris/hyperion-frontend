...
type MapFiltersProps = {
  ...
  setFilters: React.Dispatch<React.SetStateAction<{ has_trash: boolean | undefined; min_confidence: number }>>;
  items?: MapItem[];
...
const MapFilters: React.FC<MapFiltersProps> = ({ filters, setFilters, items, flyTo }) => {
  const { t } = useTranslation();
  return (
    <div className="absolute top-20 right-8 z-[1000] w-80">
      <ScrollReveal
        className="relative overflow-visible"
        delay={0.1}
      >
        {/* Decorative amorphous blobs */}
        <div className="pointer-events-none absolute -top-10 -left-10 w-32 h-32 bg-hyperion-soft-sky/30 blur-2xl animate-floaty" style={{ borderRadius: "62% 38% 70% 30% / 44% 56% 44% 56%" }} />
        <div className="pointer-events-none absolute -bottom-8 -right-8 w-28 h-28 bg-hyperion-sage-mint/25 blur-2xl animate-floaty" style={{ borderRadius: "58% 42% 36% 64% / 48% 62% 38% 52%" }} />

        <div
          className="relative bg-white/90 backdrop-blur-md border border-hyperion-muted-gold/60 shadow-[rgba(26,95,84,0.16)_0px_18px_60px] px-7 py-8 rounded-3xl space-y-7"
        >
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-3">
              <Button
                ...
                onClick={() => setFilters((f) => ({ ...f, has_trash: true }))}
              ...
              <Button
                ...
                onClick={() => setFilters((f) => ({ ...f, has_trash: false }))}
          ...
          {items && items.length > 0 && <MiniListPreview items={items} flyTo={flyTo} />}
        </div>
      </ScrollReveal>
    </div>
  );
